...
If no overrides are provided, the pipeline falls back to the OpenRouter free
router (`openrouter/free`) for both implication extraction and validation.
OpenRouter is the primary API used for those LLM calls.
...
## Troubleshooting
- Crashes on startup → verify `.env` has `OPENROUTER_API_KEY`; `IMPLICATIONS_MODEL` and `VALIDATION_MODEL` default to `openrouter/free`
