import { createCaptainLabsStore } from './captainLabsStore.js'
import { handleCaptainLabsApiRequest } from './captainLabsApi.js'
...
const PIPELINE_STATE_FILE = resolve(process.env.PIPELINE_STATE_FILE ?? `${__dirname}/../data/pipeline-state.json`)
const PIPELINE_OUTPUT_FILE = resolve(process.env.PIPELINE_OUTPUT_FILE ?? `${__dirname}/../data/pipeline-card-outputs.json`)
const CAPTAINLABS_STATE_FILE = resolve(process.env.CAPTAINLABS_STATE_FILE ?? `${__dirname}/../data/captainlabs-state.json`)
...
const captainLabsStore = createCaptainLabsStore({ filePath: CAPTAINLABS_STATE_FILE })
...
function createServer(options = {}) {
  const server = new McpServer({ name: APP_NAME, version: APP_VERSION }, { capabilities: { logging: {} } })
...
export function createHttpRequestHandler({ pipelineService = null, noteStore = null, captainLabsStore: providedStore = null } = {}) {
  ...
  return async function handleRequest(req, res) {
    ...
    if (await handleCaptainLabsApiRequest(req, res, store)) {
      return
    }
    ...
    if (req.url === '/pipeline/reset' && req.method === 'POST') {
      ...
      writeJson(res, 200, { ok: true, status: pipelineService?.getStatus?.() ?? {} })
      return
    }
    ...
    if (req.url === '/pipeline/run/production' && req.method === 'POST') {
      ...
      const runResult = pipelineService?.startProductionRun?.({
        ...
        implications_model: typeof body?.implications_model === 'string' ? body.implications_model : undefined,
        validation_model: typeof body?.validation_model === 'string' ? body.validation_model : undefined,
      })
      ...
      writeJson(res, 202, { ok: true, status: runResult.status })
      return
    }
...
async function main() {
  const handleRequest = createHttpRequestHandler({ pipelineService, noteStore, captainLabsStore })
